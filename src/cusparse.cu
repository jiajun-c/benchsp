...
int cusparse_spmv_fp16(int *hA_csrOffsets, int32_t *hA_columns, half *hA_values, half *hX, half* hY, int32_t A_num_rows, int32_t A_num_cols, int32_t A_nnz) {
    ...
    // Create dense vector X
    CHECK_CUSPARSE( cusparseCreateDnVec(&vecX, A_num_cols, dX, CUDA_R_16F) )
    // Create dense vector y
    CHECK_CUSPARSE( cusparseCreateDnVec(&vecY, A_num_rows, dY, CUDA_R_16F) )
    // allocate an external buffer if needed
    CHECK_CUSPARSE( cusparseSpMV_bufferSize(
                                 handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                 &alpha, matA, vecX, &beta, vecY, CUDA_R_32F,
...
void cusparse_spmv_fp16_warpper(int *hA_csrOffsets, int32_t *hA_columns, float *hA_values32, float *hX32, float* hY32, int32_t A_num_rows, int32_t A_num_cols, int32_t A_nnz) {
    half* hA_values = (half*)malloc(A_nnz * sizeof(half));
    half* hX = (half*)malloc(A_num_cols * sizeof(half));
    half* hY = (half*)malloc(A_num_rows * sizeof(half));
    ...
    cusparse_spmv_fp16(hA_csrOffsets, hA_columns, hA_values, hX, hY, A_num_rows, A_num_cols, A_nnz);
}
